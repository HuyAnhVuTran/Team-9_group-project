...
def collect_agent_data(
    ...
    for agent in space.agents:
        ...
        arguments["shape"].append(portray.pop("shape", shape))
        # arguments["edgecolors"].append(portray.pop("edgecolor", border_color))
        # arguments["linewidths"].append(portray.pop("linewidth", border_width))
...
def _scatter(ax: Axes, arguments, **kwargs):
    ...
    shape = arguments.pop("shape")  # Get shape from arguments
    ...
    for mark in np.unique(marker):
        ...
        for z_order in np.unique(zorder):
            ...
            for shp in np.unique(shape):
                shape_mask = shape == shp
                logical_shape = logical & shape_mask
                ...
                if shp == "square":
                    ax.scatter(
                        ...
                        zorder=z_order,
                        **{k: v[logical_shape] for k, v in arguments.items()},
                        **kwargs,
                    )
                else:
                    ax.scatter(
